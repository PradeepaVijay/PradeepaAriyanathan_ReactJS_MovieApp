...
export function* sagaWatcher() {
  ...
  yield takeEvery(FETCH_TOPRATED, fetchTopRatedMovies);
}
function* fetchTopRatedMovies() {
  try {
    const page = yield select(selector.page);
    const url = `https://api.themoviedb.org/3/movie/top_rated?api_key=${process.env.REACT_APP_SECRET_API_KEY}&language=en-US&page=${page}`;
